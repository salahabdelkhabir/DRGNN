...
class App extends React.Component<Props, State> {
  ...
  componentDidMount() {
    ...
    console.log("App - Initializing and loading resources");
    
    // Add loading status message display for user feedback
    const loadingMessages = document.createElement('div');
    loadingMessages.style.position = 'fixed';
    loadingMessages.style.top = '50%';
    loadingMessages.style.left = '50%';
    loadingMessages.style.transform = 'translate(-50%, -50%)';
    loadingMessages.style.background = 'rgba(255, 255, 255, 0.9)';
    loadingMessages.style.padding = '20px';
    loadingMessages.style.borderRadius = '5px';
    loadingMessages.style.zIndex = '9999';
    loadingMessages.innerHTML = 'Loading application resources...';
    document.body.appendChild(loadingMessages);
  
    const updateLoadingMessage = (message: string) => {
      loadingMessages.innerHTML = message;
    };
  
    requestNodeTypes()
      .then((nodeTypes) => {
        updateLoadingMessage('Loading node types...');
      ...
      .then(() => {
        updateLoadingMessage('Loading edge types...');
        return requestEdgeTypes();
      })
      .then((edgeTypes) => {
        this.props.dispatch({
      ...
      .then(() => {
        updateLoadingMessage('Loading node names...');
        return requestNodeNameDict();
      })
      .then((nodeNameDict) => {
        this.props.dispatch({
      ...
      .then(() => {
        updateLoadingMessage('Loading disease options...');
        return requestDiseaseOptions();
      })
      .then((diseaseOptions) => {
        console.log(`App - Loaded ${diseaseOptions.length} disease options`);
        this.props.dispatch({
          type: ACTION_TYPES.Load_Disease_Options,
          payload: { diseaseOptions },
        });
        
        // Check if we have valid disease options before proceeding
        if (!Array.isArray(diseaseOptions) || diseaseOptions.length === 0) {
          console.error("App - No disease options available, cannot initialize properly");
          throw new Error("Failed to load disease options");
        }
        
        // Use the first available disease instead of INIT_DISEASE if INIT_DISEASE is empty
        const initialDisease = INIT_DISEASE || (diseaseOptions.length > 0 ? diseaseOptions[0][0] : '');
        updateLoadingMessage(`Loading disease data for ${initialDisease}...`);
        
        if (!initialDisease) {
          console.error("App - No initial disease available, cannot proceed");
          throw new Error("No valid disease ID available for initialization");
        }
        
        console.log(`App - Using initial disease: ${initialDisease}`);
        return selectDisease(initialDisease, this.props.dispatch);
      })
      .then(() => {
        // Only try to select drugs if we have valid initial disease and drugs
        if (INIT_DRUGS && INIT_DRUGS.length > 0 && this.props.globalState.selectedDisease) {
          updateLoadingMessage('Loading drug data...');
          const promises = INIT_DRUGS.map(drugId => 
      ...
      .then(() => {
        updateLoadingMessage('Application loaded successfully');
        setTimeout(() => {
          document.body.removeChild(loadingMessages);
        }, 1000);
      ...
      .catch(error => {
        ...
        updateLoadingMessage(`Error loading application: ${error.message}. Please check console for details.`);
