...
export default class AttentionTree extends React.Component<Props, {}> {
  ...
  drawNodeAttentionHorizontal(
    ...
    // Add null check and provide default values if nodeAttention is undefined
    if (!nodeAttention) {
      console.warn('Node attention data is undefined');
      // Return an empty content and zero height
      return {
        content: [],
        height: 0
      };
    ...
    const nodes = root.descendants().map((node, i) => {
      ...
      // Add defensive checks for nodeType and nodeNameDict
      if (!nodeType || !nodeNameDict || !nodeNameDict[nodeType]) {
        console.warn(`Missing nodeType or nodeNameDict for node: ${nodeId}, type: ${nodeType}`);
        return (
          <g key={`node${i}_unknown`} className="missing-node">
            <rect
      ...
      if (nodeFullName === undefined) {
        // Try to fix merged node ID
        try {
          nodeId = nodeId.replace(/_/g, '') + '.0'; // the id of a merged node is xxx_xxx_xxxx
          nodeFullName = nodeNameDict[nodeType][nodeId];
        } catch (e) {
          console.warn(`Failed to find name for node: ${nodeId}, type: ${nodeType}`);
          nodeFullName = `${nodeType}:${nodeId}`;
        }
      }
      ...
      return (
        <Tooltip
          title={tooltipTitle}
          key={`node${i}_${nodeFullName || nodeId}`}
          destroyTooltipOnHide
          mouseEnterDelay={0.3}
        >
          <g
  ...
  drawSubgraph() {
    ...
    if (!attention || typeof attention !== 'object' || Object.keys(attention).length === 0) {
      console.warn('No attention data available');
    ...
    try {
      const content = Object.keys(attention).map((nodeKey: string, idx) => {
        try {
          ...
          console.error(`Error drawing node attention for ${nodeKey}:`, error);
          heights.push(0);
          return <g key={`error_${nodeKey}`}></g>;
        }
      });
      return { content, height: Math.max(...heights, 1) }; // Ensure height is at least 1
    } catch (error) {
      console.error('Error in drawSubgraph:', error);
      return { content: [], height: 0 };
    }
  }
  render() {
