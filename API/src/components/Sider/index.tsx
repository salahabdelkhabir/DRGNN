  ...
  render() {
    ...
    let sider = (
      ...
      >
        Disease:
        <Select
          placeholder="Select a disease to explore"
        ...
        >
          {diseaseOptions.length > 0 ? (
            diseaseOptions
              .filter((d) => {
                // Add a safety check for nodeNameDict and the disease name
                const diseaseId = d[0];
                const diseaseName = nodeNameDict['disease']?.[diseaseId];
                return diseaseName && !wheterRemoveDisease(diseaseName);
              })
              .map((d) => {
                const [id, treatable] = d;
                // Add additional safety checks
                const name = nodeNameDict['disease']?.[id] || 'Unknown Disease';
                const cropName =
