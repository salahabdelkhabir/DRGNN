...
class NodeLink extends React.Component<Props, State> {
  ...
  render() {
    ...
    const cardWidth = width - 2 * this.margin - 2 * this.padding,
      cardHeight = height - 2 * this.padding - this.titleHeight - 2 * this.margin;
  
    const props = { ...this.props, height: cardHeight, width: cardWidth };
    
    // Check if a disease is selected
    const noDiseaseSelected = !globalState.selectedDisease;
    ...
    >
      {noDiseaseSelected ? (
        <div style={{ 
          ...
          <h3>Welcome to TxGNN Explorer</h3>
          <p>Please select a disease and a drug from the left panel to start exploring.</p>
        </div>
      ) : globalState.drugPredictions.length === 0 ? (
        <div style={{ 
          display: 'flex', 
          justifyContent: 'center', 
          alignItems: 'center', 
          ...
          <p>No drug predictions found for the selected disease. This may be due to:</p>
          <ul>
            <li>Missing data files in the server</li>
            <li>The disease has no associated drug predictions</li>
            <li>An error occurred while loading the data</li>
          </ul>
          <p>Please try selecting a different disease or check the console for errors.</p>
        </div>
      ) : (
        <PathMatrix {...props} />
      )}
