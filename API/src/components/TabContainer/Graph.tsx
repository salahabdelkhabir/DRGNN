...
export default class ModelNodeForce extends React.Component<Props, State> {
  ...
  getNodeLinks() {
    ...
    if (!attention || Object.keys(attention).length === 0) {
      console.warn("No attention data available for graph");
      return { nodes: [], links: [] };
    }
    ...
    try {
      Object.values(attention).forEach((nodeAttention: IAttentionTree) => {
        // Check if nodeAttention is valid
        if (!nodeAttention || !nodeAttention.children) {
          console.warn("Invalid node attention data", nodeAttention);
          return;
        }
        ...
        rootNode.descendants().forEach((d) => {
          ...
          const prevNode = this.prevNodes.find((prevD) => prevD.id === node.id);
        ...
        const desLinks = rootNode.links().map((d) => {
          ...
          edgeInfo = edgeTypes && edgeTypes[edgeInfo]?.edgeInfo || edgeInfo;
          return {
      ...
      // Position disease and drug nodes
  ...
  drawGraph() {
    ...
    if (!nodes || nodes.length === 0) {
      console.warn("No nodes data available for graph");
      return;
    }
    ...
    // Check if nodeNameDict is properly initialized before adding labels
    const addLabels = nodeNameDict && Object.keys(nodeNameDict).length > 0;
    ...
    if (addLabels) {
      ...
      svgNodes
        ...
        .text((d) => {
          try {
            // Safely access node name with fallbacks
            if (nodeNameDict && d.nodeType && nodeNameDict[d.nodeType]) {
              const name = nodeNameDict[d.nodeType][d.nodeId];
              return name || `${d.nodeType}:${d.nodeId}`;
            }
            return d.nodeId || "Unknown";
          } catch (e) {
            console.warn(`Error getting node label for ${d.nodeId}: ${e}`);
            return d.nodeId || "Unknown";
          }
        })
        .classed('hidden', true);

      // Show labels for target nodes or highlighted nodes
      svgNodes
      ...
      console.warn("nodeNameDict is not available, skipping node labels");
    }
