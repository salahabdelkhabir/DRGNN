...
def construct_negative_graph_each_etype(graph, k, etype, method, weights, device):
    ...
    if method == 'corrupt_dst':
        ...
        try:
            ...
            neg_src = torch.Tensor([]).to(device)  # Ensure tensors are on the right device
        ...
        try:
            ...
            neg_dst = torch.Tensor([]).to(device)  # Ensure tensors are on the right device
...
class Full_Graph_NegSampler:
    ...
    def __call__(self, graph):
        ...
        for etype in graph.canonical_etypes:
            ...
            # Ensure `temp` has the expected structure and is non-empty
            if etype in temp and len(temp[etype][0]) != 0:
                out.update(temp)
        
        # Ensure `out` is not empty before constructing the heterograph
        if not out:
            raise ValueError("No negative edges were constructed for the given graph.")
        
        return dgl.heterograph(out, num_nodes_dict={ntype: graph.number_of_nodes(ntype) for ntype in graph.ntypes}, device=device)
...
def evaluate_graph_construct(df_valid, g, neg_sampler, k, device):
    ...
    g_valid = dgl.heterograph(out, num_nodes_dict={ntype: g.number_of_nodes(ntype) for ntype in g.ntypes}, device=device)
    g_valid = g_valid.to(device)
    print(f"[evaluate_graph_construct] g_valid device: {g_valid.device}")
...
def get_all_metrics_fb(pred_score_pos, pred_score_neg, scores, labels, G, full_mode = False):
    ...
    for etype in etypes:
        ...
        try:
            out_pos = pred_score_pos[etype].reshape(-1,).detach().to(device).numpy()
.numpy()
            out_neg = pred_score_neg[etype].reshape(-1,).detach().to(device).numpy()
...
def evaluate(model, valid_data, G):
    ...
    return get_all_metrics(valid_data.label.values, scores.to(device).detach().numpy(), rels)
...
def evaluate_fb(model, g_pos, g_neg, G, dd_etypes, device, return_embed = False, mode = 'valid'):
    ...
    loss = F.binary_cross_entropy(scores, torch.tensor(labels, dtype=torch.float32, device=device))
            
    if return_embed:
        return get_all_metrics_fb(pred_score_pos, pred_score_neg, scores.reshape(-1,).detach().to(device).numpy(), labels, G, True), loss.item(), pred_score_pos, pred_score_neg
    else:
        return get_all_metrics_fb(pred_score_pos, pred_score_neg, scores.reshape(-1,).detach().to(device).numpy(), labels, G, True), loss.item()
...
def evaluate_gnnexplainer(model, G, g_valid_pos, g_valid_neg, only_relation, epoch, etypes_train, penalty_scaling, device, mode = 'validation', weight_bias_track = False, wandb = None):
    ...
    with torch.no_grad():
        ...
        original_predictions = torch.sigmoid(torch.cat((pos_score, neg_score))).to(self.device)
...
def evaluate_graphmask(model, G, g_valid_pos, g_valid_neg, only_relation, epoch, etypes_train, allowance, penalty_scaling, device, mode = 'validation', weight_bias_track = False, wandb = None, no_base = False):
    ...
    with torch.no_grad():
        ...
        g_valid_pos = g_valid_pos.to(self.device)
        g_valid_neg = g_valid_neg.to(self.device)
        ...
        if mode == 'testing':
            test_metrics = {}
            pred_update = updated_predictions.detach().to(device).numpy()
            pred_ori = original_predictions.detach().to(device).numpy()
            y_ = np.array(labels)
...
def print_dict(x, required_relations):
...
def disease_centric_evaluation(df, df_train, df_valid, df_test, data_path, G, model, device, disease_ids = None, relation = None, weight_bias_track = False, wandb = None, show_plot = False, verbose = False, return_raw = False, simulate_random = True, only_prediction = False):
    G = G.to(device)
    print(f"[evaluate_graph_construct] Graph moved to device: {g.device}")
    ...
    def get_scores_disease(rel, disease_ids):
        ...
        drug_nodes = G.nodes('drug').to(device).numpy()
        ...
        for disease_id in tqdm(disease_ids):
            ...
            pred = pred_score_rel[('disease', rel, 'drug')].reshape(-1,).detach().to(device).numpy()
            lab = {idx2id_drug[i]: labels[i] for i in g_eval.edges()[1].detach().to(device).numpy()}
            preds_contra[idx2id_disease[disease_id]] = {idx2id_drug[i]: pred[idx] for idx, i in enumerate(g_eval.edges()[1].detach().to(device).numpy())}
            labels_contra[idx2id_disease[disease_id]] = lab
            ids_contra[idx2id_disease[disease_id]] = g_eval.edges()[1].detach().to(device).numpy()
...
def randomize_edges(hetero_graph):
    ...
    for etype in hetero_graph.canonical_etypes:
        ...
        src_tensor = torch.tensor(src_random_nodes, dtype=torch.int64).to(hetero_graph.device)
        dst_tensor = torch.tensor(dst_random_nodes, dtype=torch.int64).to(hetero_graph.device)
        src_tensor = src_tensor.to(hetero_graph.device)
        dst_tensor = dst_tensor.to(hetero_graph.device)
